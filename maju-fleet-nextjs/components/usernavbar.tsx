...
export default function UserNavbar() {
  ...
  const navLinks = [
    ...
    { name: "MY SHIPMENTS", href: "/dashboard/myshipments" },
    { name: "BILLING & HISTORY", href: "/dashboard/billing" },
  ];
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                key={link.name}
                href={link.href}
                className={`font-grotesk text-[13px] uppercase tracking-[2px] pb-2 border-b-2 transition-all duration-300 ${
                  isActive
                    ? "font-bold text-[#E5B5FF] border-[#B026FF] [text-shadow:-1px_0_0_rgba(255,0,0,0.6),1px_0_0_rgba(0,255,255,0.6)]"
                    : "text-white/60 border-transparent hover:text-[#E5B5FF] hover:border-[#B026FF]"
                }`}
              >
                {link.name}
              </Link>
        ...
        {showLogoutModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative bg-[#0a0a0c] border border-[#B026FF]/30 rounded-xl p-8 max-w-sm w-full shadow-[0_0_50px_rgba(176,38,255,0.15)] z-10 text-center">
              <h2 className="text-white font-grotesk font-bold text-xl tracking-[2px] uppercase mb-3">Exit Portal</h2>
              <p className="text-white/60 font-mono text-[11px] tracking-widest mb-8 leading-relaxed">Are you sure you want to log out and return to the main landing page?</p>
