...
function AdminControlContent() {
  ...
  const executeDeleteData = async () => {
    ...
    try {
      ...
      if (result.success) {
        showNotification("success", "RECORD PURGED", `Data matrix sequence has been successfully processed.`);
        fetchNeonData(); 
        setViewMode("table");
      } else {
    ...
    if (activeTab === "fleet") {
      ...
      if (formData.vesselId) {
        ...
        if (selectedVessel && cargoWeight > selectedVessel.capacity) {
          errors.vesselId = `OVERLOAD CLEARANCE DENIED: Cargo weight (${cargoWeight} KG) exceeds selected vessel max payload capability (${selectedVessel.capacity} ${selectedVessel.capacityUnit || 'KG/TONNES'}).`;
        }
      }
  ...
  const PaginationBar = ({ total }: { total: number }) => {
    ...
    return (
      <div className="flex justify-end items-center gap-4 px-4 py-4 border-t border-white/5 font-mono text-[11px] tracking-widest uppercase text-white/40">
        <button onClick={() => setCurrentPage(p => Math.max(p - 1, 1))} disabled={currentPage === 1} className="hover:text-[#E5B5FF] disabled:opacity-30 transition-colors cursor-pointer disabled:cursor-not-allowed">Previous</button>
        ...
        <button onClick={() => setCurrentPage(p => Math.min(p + 1, totalPages))} disabled={currentPage === totalPages} className="hover:text-[#E5B5FF] disabled:opacity-30 transition-colors cursor-pointer disabled:cursor-not-allowed">Next</button>
      </div>
    );
  };
    ...
    <main className="w-full px-6 md:px-10 relative z-10 flex flex-col gap-8 max-w-[1200px] mx-auto pb-10 mt-6">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="bg-transparent pb-4">
        ...
        <div className="mb-10">
          ...
          <h1 className="font-grotesk font-bold text-[32px] md:text-[42px] tracking-[2px] uppercase text-white border-b-4 border-[#B026FF] pb-2 inline-block">{getTitle()}</h1>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between gap-4 mb-8 bg-[#121317] p-4 rounded-lg border border-white/5">
          <div className="flex flex-1 flex-col sm:flex-row gap-4">
            <div className="relative w-full md:w-[360px]">
              ...
              <input type="text" placeholder={`Search in ${activeTab}...`} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-[#1a1b22] border-none rounded py-2.5 pl-10 pr-4 text-sm text-white placeholder:text-white/30 focus:ring-1 focus:ring-[#B026FF] outline-none transition-all" />
            </div>

            {activeTab === "fleet" && viewMode === "table" && (
              <div className="relative">
                <select value={fleetStatusFilter} onChange={(e) => setFleetStatusFilter(e.target.value)} className="bg-[#1a1b22] border border-white/10 rounded py-2.5 pl-10 pr-10 text-xs font-mono tracking-widest text-[#E5B5FF] appearance-none outline-none cursor-pointer focus:border-[#B026FF] uppercase h-full">
        ...
        <AnimatePresence mode="wait">
          {isLoading ? (
            ...
            <motion.div key="table-view" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} transition={{ duration: 0.2 }} className="bg-[#121317] rounded-lg border border-white/5 overflow-hidden">
                          ...
                          <tr>
                            <td colSpan={4}>
                              <div className="flex flex-col items-center justify-center py-20">
                                ...
                                <p className="text-white/40 font-mono tracking-widest text-[12px] mb-4 uppercase">{searchQuery ? `NO RESULTS FOUND FOR "${searchQuery}"` : "NO FLEET DATA AVAILABLE."}</p>
                              </div>
                            </td>
                          </tr>
                        );
              ...
              {activeTab === "fleet" && <PaginationBar total={shipments.length} />}
              {activeTab === "customer" && <PaginationBar total={customers.length} />}
              {activeTab === "crew" && <PaginationBar total={crews.length} />}
              {activeTab === "vessel" && <PaginationBar total={vessels.length} />}
            </motion.div>
          ) : (
            <motion.div key="form-view" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.3 }} className="max-w-[1000px] w-full">
              <form onSubmit={handleSave} className="w-full" noValidate>
                
                {activeTab === "fleet" && (
                  <div className="flex flex-col gap-10">
                    <div>
                      <h2 className="flex items-center gap-4 font-grotesk font-bold text-[#E5B5FF] uppercase tracking-[3px] mb-6 text-lg"><Hash size={20} /> TRACKING CODE & SYSTEM STATUS</h2>
                      <div className="flex gap-6 items-end">
                        ...
                        <div className="flex-1">
                          <label className="text-[12px] font-bold text-white/40 tracking-[3px] uppercase mb-3 block font-mono">LOGISTICS STATUS CONTROL</label>
                          <div className="relative">
                            <select value={formData.status || "PENDING"} onChange={e => updateField("status", e.target.value)} className={getSelectClass("status")}>
                              {currentAvailableStatuses.map(st => (
                        ...
                        <div>
                          ...
                          <div className="relative">
                            ...
                            >
                              ...
                              {vessels.map(v => (<option key={v.id} value={v.id}>{v.name} (Max: {v.capacity} {v.capacityUnit})</option>))}
                            </select>
                            <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-white/40"><ChevronDown size={20} /></div>
                          </div>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-x-8 gap-y-8">
                        <div>
                          <label className="text-[12px] font-bold tracking-[3px] uppercase mb-3 block font-mono text-white/40">NAME</label>
                          <input type="text" readOnly value={formData.senderName || ""} className={`${getInputClass("senderName")} !text-white/60 !border-white/10 cursor-not-allowed bg-white/[0.02] rounded px-3 py-1`} />
                        </div>
                        <div>
                          <label className="text-[12px] font-bold tracking-[3px] uppercase mb-3 block font-mono text-white/40">CONTACT</label>
                          <input type="text" readOnly value={formData.senderContact || ""} className={`${getInputClass("senderContact")} !text-white/60 !border-white/10 cursor-not-allowed bg-white/[0.02] rounded px-3 py-1`} />
                        </div>
                        <div className="col-span-2">
                          <label className="text-[12px] font-bold tracking-[3px] uppercase mb-3 block font-mono text-white/40">EMAIL ADDRESS</label>
                          <input type="email" readOnly value={formData.senderEmail || ""} className={`${getInputClass("senderEmail")} !text-white/60 !border-white/10 cursor-not-allowed bg-white/[0.02] rounded px-3 py-1`} />
                        </div>
                        <div className="col-span-2">
                          <label className="text-[12px] font-bold tracking-[3px] uppercase mb-3 block font-mono text-white/40">ORIGIN ADDRESS</label>
                          <input type="text" readOnly value={formData.senderAddress || ""} className={`${getInputClass("senderAddress")} !text-white/60 !border-white/10 cursor-not-allowed bg-white/[0.02] rounded px-3 py-1`} />
                        </div>
                        <div>
                          <label className="text-[12px] font-bold tracking-[3px] uppercase mb-3 block font-mono text-white/40">COUNTRY</label>
                          <input type="text" readOnly value={formData.senderCountry || ""} className={`${getInputClass("senderCountry")} !text-white/60 !border-white/10 cursor-not-allowed bg-white/[0.02] rounded px-3 py-1`} />
                        </div>
                        <div>
                          <label className="text-[12px] font-bold tracking-[3px] uppercase mb-3 block font-mono text-white/40">PROVINCE / CITY</label>
                          <input type="text" readOnly value={formData.senderCity || ""} className={`${getInputClass("senderCity")} !text-white/60 !border-white/10 cursor-not-allowed bg-white/[0.02] rounded px-3 py-1`} />
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-x-8 gap-y-8">
                        <div>
                          <label className="text-[12px] font-bold tracking-[3px] uppercase mb-3 block font-mono text-white/40">NAME</label>
                          <input type="text" readOnly value={formData.recipientName || ""} className={`${getInputClass("recipientName")} !text-white/60 !border-white/10 cursor-not-allowed bg-white/[0.02] rounded px-3 py-1`} />
                        </div>
                        <div>
                          <label className="text-[12px] font-bold tracking-[3px] uppercase mb-3 block font-mono text-white/40">CONTACT</label>
                          <input type="text" readOnly value={formData.recipientContact || ""} className={`${getInputClass("recipientContact")} !text-white/60 !border-white/10 cursor-not-allowed bg-white/[0.02] rounded px-3 py-1`} />
                        </div>
                        <div className="col-span-2">
                          <label className="text-[12px] font-bold tracking-[3px] uppercase mb-3 block font-mono text-white/40">EMAIL ADDRESS</label>
                          <input type="email" readOnly value={formData.recipientEmail || ""} className={`${getInputClass("recipientEmail")} !text-white/60 !border-white/10 cursor-not-allowed bg-white/[0.02] rounded px-3 py-1`} />
                        </div>
                        <div className="col-span-2">
                          <label className="text-[12px] font-bold tracking-[3px] uppercase mb-3 block font-mono text-white/40">DESTINATION ADDRESS</label>
                          <input type="text" readOnly value={formData.recipientAddress || ""} className={`${getInputClass("recipientAddress")} !text-white/60 !border-white/10 cursor-not-allowed bg-white/[0.02] rounded px-3 py-1`} />
                        </div>
                        <div>
                          <label className="text-[12px] font-bold tracking-[3px] uppercase mb-3 block font-mono text-white/40">COUNTRY</label>
                          <input type="text" readOnly value={formData.recipientCountry || ""} className={`${getInputClass("recipientCountry")} !text-white/60 !border-white/10 cursor-not-allowed bg-white/[0.02] rounded px-3 py-1`} />
                        </div>
                        <div>
                          <label className="text-[12px] font-bold tracking-[3px] uppercase mb-3 block font-mono text-white/40">PROVINCE / CITY</label>
                          <input type="text" readOnly value={formData.recipientCity || ""} className={`${getInputClass("recipientCity")} !text-white/60 !border-white/10 cursor-not-allowed bg-white/[0.02] rounded px-3 py-1`} />
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-col gap-8">
                        <div className="w-full">
                          <label className="text-[12px] font-bold tracking-[3px] uppercase mb-3 block font-mono text-white/40">ITEM DESCRIPTION</label>
                          <input type="text" readOnly value={formData.cargoDesc || ""} className="w-full bg-[#121317]/40 border-b border-white/20 pb-3 text-[18px] text-white/60 outline-none cursor-not-allowed px-3" />
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 items-start">
                          <div className="flex flex-col gap-2">
                            <label className="text-[12px] font-bold tracking-[3px] uppercase mb-1 block font-mono text-white/40">TOTAL WEIGHT</label>
                            <div className="flex gap-2 items-center relative border-b border-white/20 bg-[#121317]/40 px-3 cursor-not-allowed">
                              <input type="number" readOnly value={formData.weight || ""} className="w-full bg-transparent border-none pb-3 text-[18px] text-white/60 outline-none cursor-not-allowed" />
                              <select disabled value={formData.weightUnit || "KG"} className="bg-[#1a1b22] border-none rounded py-1 px-3 mb-2 text-xs font-bold text-[#E5B5FF] appearance-none outline-none pr-7 cursor-not-allowed">
                                <option value="KG">KG</option><option value="TON">TON</option><option value="LBS">LBS</option>
                              </select>
                            </div>
                          </div>
                          <div className="flex flex-col gap-2">
                            <label className="text-[12px] font-bold tracking-[3px] uppercase mb-1 block font-mono text-white/40">DIMENSIONS (CBM)</label>
                            <input type="number" readOnly value={formData.dimensions || ""} className="w-full bg-[#121317]/40 border-b border-white/20 pb-3 text-[18px] text-white/60 outline-none cursor-not-allowed px-3" />
                          </div>
                          <div className="flex flex-col gap-2">
                            <label className="text-[12px] font-bold tracking-[3px] uppercase mb-1 block font-mono text-white/40">CATEGORY</label>
                            <div className="relative opacity-50 cursor-not-allowed">
                              <select disabled value={formData.category || ""} className={getSelectClass("category")}>
                          ...
                          <div className="flex flex-col gap-2">
                            <label className="text-[12px] font-bold tracking-[3px] uppercase mb-1 block font-mono text-white/40">SERVICE PLAN</label>
                            <div className="relative opacity-50 cursor-not-allowed">
                              <select disabled value={formData.packageTypeId || "2"} className={getSelectClass("packageTypeId")}>
                ...
                {activeTab === "crew" && (
                  <div className="flex flex-col gap-10">
                    ...
                    <div>
                      <h2 className="flex items-center gap-4 font-grotesk font-bold text-[#E5B5FF] uppercase tracking-[3px] mb-6 text-lg"><Users size={20} /> PERSONAL DETAILS</h2>
                      <div className="grid grid-cols-2 gap-x-8 gap-y-8">
                        <div className="col-span-2">
                          ...
                          <input type="text" disabled={isUpdating} value={formData.name || ""} onChange={e => updateField("name", e.target.value)} className={`${getInputClass("name")} ${isUpdating ? 'opacity-50 cursor-not-allowed' : ''}`} />
                        ...
                        <div className="col-span-2">
                          ...
                          <input type="email" disabled={isUpdating} value={formData.email || ""} onChange={e => updateField("email", e.target.value)} className={`${getInputClass("email")} ${isUpdating ? 'opacity-50 cursor-not-allowed' : ''}`} />
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <select disabled={isUpdating} value={formData.gender || ""} onChange={e => updateField("gender", e.target.value)} className={`${getSelectClass("gender")} ${isUpdating ? 'opacity-50 cursor-not-allowed' : ''}`}>
                        ...
                        <div>
                          ...
                          <input type="number" disabled={isUpdating} value={formData.age || ""} onChange={e => updateField("age", e.target.value)} className={`${getInputClass("age")} ${isUpdating ? 'opacity-50 cursor-not-allowed' : ''}`} />
                        </div>
                      </div>
                    </div>
                    <div>
                      <h2 className="flex items-center gap-4 font-grotesk font-bold text-[#E5B5FF] uppercase tracking-[3px] mb-6 text-lg"><Briefcase size={20} /> PROFESSIONAL DETAILS</h2>
                      <div className="grid grid-cols-2 gap-x-8 gap-y-8">
                        <div>
                          ...
                          <div className="relative">
                            <select value={formData.role || ""} onChange={e => updateField("role", e.target.value)} className={getSelectClass("role")}>
                              ...
                              <option value="captain">Captain</option><option value="navigator">Navigator</option><option value="chief_engineer">Chief Engineer</option><option value="deck_officer">Deck Officer</option>
                            </select>
                            <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-white/40"><ChevronDown size={20} /></div>
                          </div>
                        </div>
                ...
                {activeTab === "vessel" && (
                  <div className="flex flex-col gap-10">
                    <div>
                      <h2 className="flex items-center gap-4 font-grotesk font-bold text-[#E5B5FF] uppercase tracking-[3px] mb-6 text-lg"><Users size={20} /> VESSEL SPECIFICATIONS</h2>
                      <div className="grid grid-cols-2 gap-x-8 gap-y-8">
                        <div className="col-span-2">
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <div className={`flex gap-2 items-center relative border-b transition-colors ${formErrors.capacity ? 'border-[#FF3B30]' : 'border-white/20 focus-within:border-[#B026FF]'}`}>
                            ...
                            <select value={formData.capacityUnit || "TONNES"} onChange={e => updateField("capacityUnit", e.target.value)} className="bg-[#1a1b22] border-none rounded py-1 px-3 mb-2 text-xs font-bold text-[#E5B5FF] appearance-none outline-none pr-7 cursor-pointer">
                              <option value="TONNES">TONNES</option><option value="KG">KG</option><option value="CBM">CBM</option>
                            </select>
                          </div>
                        </div>
                        <div className="flex flex-col gap-2">
                        ...
                        <div className="col-span-2">
                          ...
                          <div className="relative">
                            <select 
                              ...
                              className={getSelectClass("crewLead")}
                            >
                              <option value="" disabled hidden>Select crew leader from list...</option>
                              {crews.map(cr => (
                                <option key={cr.id} value={cr.name}>{cr.name} ({cr.role ? cr.role.replace(/_/g, ' ').toUpperCase() : "CREW"})</option>
                              ))}
                            </select>
                            <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-white/40"><ChevronDown size={20} /></div>
                          </div>
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <select value={formData.status || "Active"} onChange={e => updateField("status", e.target.value)} className={getSelectClass("status")}>
                              <option value="Active">Active</option><option value="Maintenance">Maintenance</option><option value="Docked">Docked</option>
                            </select>
                            <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-white/40"><ChevronDown size={20} /></div>
                          </div>
                        </div>
